...
- **FR-063**: Each mutator MUST run inside exactly one database transaction — one transaction in the database's own sense, not one logical operation spanning several. This is the boundary requirement only; the serialization each transaction MUST provide is fixed where its mechanism is: `createIssue`'s counter draw by FR-013, whose lock is held across the read and the write, with FR-016 stating the outcome; `updateIssue`'s read of the stored row and its write by FR-055, which requires both inside one transaction. What this requirement adds to those is that neither mutator MUST be allowed to span two transactions or to run outside one, and that `deleteIssue` MUST hold its cascade in a single transaction under FR-058. No isolation level is prescribed. (`OT-DATA-012`, `OT-DATA-008`, §5)
...
- **A blur on a field the user did not change issues no call at all.** `OT-UX-009` fixes one mutator call per field but does not say whether a blur on an untouched field is one of them. This is no longer an assumption on either half: FR-055 requires a call that changes nothing to write nothing, and FR-048 now requires the client not to make the call at all. It is recorded here because the reading of `OT-UX-009` that produced it was a choice — the source admits a reading in which every blur is a call — and because that reading, not the requirement, is what a later amendment would revisit.
