...
Six items where the design does not sit cleanly inside a principle, or reaches into work another
...
| **R5's `isAdmin` predicate is exported from `authorization.ts`** (a reach-back into an inherited module) | `deleteIssue`'s `isAdmin` check and the rail's `canDelete` capability both need to ask whether the actor administers the project, and R5's `authorization.ts` already computes exactly that from `actor.role`. Exporting the existing one-line function is the change; nothing about its logic moves or changes. | *Duplicate the check in `issue-queries.ts`.* `actor.role === "admin"` copied into a second file is the kind of drift Principle I and III both refuse — one definition of who administers a project, not two that could disagree. *Route delete-authorization through `isMember` instead.* `isMember` admits every admin, but admits members too; `canDelete` needs the narrower predicate `isMember` was never built to express. |
