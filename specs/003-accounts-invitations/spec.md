...
3. **Given** an address that already has an account, **When** it is entered and the field loses focus, **Then** an inline error names that account and offers a route to its row on the Accounts tab, and no invitation is created.
...
- **An address invited twice in quick succession by two admins**: the second attempt must find the first and offer resend rather than write a second invitation for one address.
...
- **FR-005**: An **Invite** control at the head of the Invitations tab MUST open a modal carrying one email field and a submit control, and nothing else. (§3.9)
- **FR-006**: The field MUST be validated per field and on blur rather than as a wall of errors on submit, and the submit control MUST stay enabled and report what is wrong inline rather than going dead. (`OT-UX-011`)
- **FR-007**: A malformed address MUST be refused inline. (§3.9)
- **FR-008**: An address that already has an account MUST be refused inline, naming that account and offering a route to its row on the Accounts tab. (§3.9, §3.12)
...
- **FR-011**: Submitting MUST run one `inviteUser` call, close the modal and place the new invitation at the head of the list; Cancel or Escape MUST close the modal, discard the field and write nothing. (§3.9)
...
- **FR-018**: The tab MUST list every invitation not yet accepted or revoked, newest first, each showing the address, who invited them, when it was sent and when it expires. (§3.9)
...
- **FR-022**: An expired invitation MUST stay listed, marked expired, with resend still offered. (§3.9)
...
- **FR-034**: Acceptance MUST be refused where the invited address has since acquired an account, so one address can never yield two accounts. (`OT-INV-016`)
...
- **FR-036**: The Accounts tab MUST list every account on the team, active accounts first and closed accounts after them. (§3.9)
...
- **FR-044**: Each MUST ask for confirmation once, and deactivation's confirmation MUST name what stays. (§3.9)
...
- **FR-050**: On the last active admin's row, the Deactivate control MUST render disabled with its reason stated inline, and MUST NOT be hidden. (§3.9, `OT-UX-002`)
...
#### Cross-cutting UX conventions this screen honours

Entry R2 fixes these four as rules and writes no code for them, having no writing or data-loading surface to exercise them. This feature has both, so it honours all four. They are **not one obligation of the same kind and not a transferable bundle**: the toast host and the connection banner are single app-wide instances that live in the shell and that no entry owns, consumed here and by every later screen; the skeleton and re-query are per-screen work this feature does for its own screens. Nothing below turns on whether R3 or R4 is built first, and neither entry establishes these for the other — see *Assumptions → Reconciliations*. No user journey above is *about* them, but each is exercised by one and carries its acceptance scenarios there: story 1 scenarios 12 to 14 for the toast, the in-flight state, the skeleton and the connection banner; story 3 scenario 9 for re-query on return; story 4 scenario 13 for a rejected write.

- **FR-054**: Toasts MUST be four kinds — success, info, warning, error — rendered top-right, stacked and auto-dismissing. The host that renders and stacks them MUST be a single app-wide instance living in the shell; this feature MUST raise toasts into it and MUST NOT stand up a second host. Whether it already exists when these screens are built or arrives with them, no entry owns it. (`OT-UX-016`)
- **FR-055**: Loading MUST use per-screen skeletons matching the layout they replace; a full-screen spinner MUST NOT be used and data landing MUST NOT shift layout. This feature MUST author its own: a skeleton matches the layout it replaces, so there is no shared component to inherit and none to extract. (`OT-UX-005`)
- **FR-056**: A revisited screen MUST re-query the server; nothing MUST render from a client cache. This is a data-fetching posture this feature adopts for its own screens, not a component; it holds identically whichever entry is built first. (`OT-UX-006`)
- **FR-057**: A lost connection MUST show one banner and MUST refuse writes with "Changes need a connection"; nothing MUST be queued for later. The banner MUST likewise be a single app-wide instance in the shell, one for the whole application rather than one per screen, consumed here on the same terms as the toast host in FR-054. (`OT-UX-017`)
...
- **Invitation** — a standing offer of a login to one address. Carries the address, the admin who issued it, when it was issued, when it expires, whether it has been spent, and the digest of its single-use token. Exactly one live link at a time: reissuing replaces it. Dropped by revoke; retained after acceptance, so a spent link can be told from an unknown one. Its secret is never reachable from a read endpoint.
...
- **SC-004**: A person landing on a dead link can tell which of the three things happened — used, expired, or not a link this installation issued — without asking an admin.
...
- **SC-009**: Reopening a closed account restores exactly what it had — the same access, with no new link, no re-invitation and nothing for the account holder to do.
...
- **Acceptance retains the invitation, marked spent; only revoke deletes a row.** This is forced rather than chosen: telling a used link from an unknown one (§3.1) requires the row to survive acceptance. A retained spent row leaves the list, which shows only invitations "not yet accepted or revoked".
...
- **The four cross-cutting UX conventions are split by kind, not assigned by build order.** R2's boundary defers toasts, per-screen skeletons, re-query on navigation and the connection-lost banner to "R3 or R4, whichever is built first". Treating them as one transferable bundle is what made that a standoff between this entry and R4, and they are not one kind of thing. The toast host and the connection banner are single app-wide instances belonging to the shell, consumed by every screen and owned by none, so neither entry establishes them for the other; whichever entry has the first surface mounts them once in the shell layout, and the other consumes what it finds. The skeleton is per-screen by its own definition — it matches the layout it replaces — and re-query is a fetching posture, so this feature authors both for its own screens in either order. Nothing here turns on which entry lands first, and FR-054 to FR-057 stay obligations on these screens under every build order.
- **The "link to it" beside an already-registered address points at that account's row on the Accounts tab.** §3.9 offers "a link to it"; §3.12 states there is no route to view or edit anyone else's profile. The Accounts roster is the only surface in the product that shows another user's account, so it is what the link reaches.
