...
Entry R2 fixes these four as rules and writes no code for them, having no writing or data-loading surface to exercise them; they land with the first slice that has one. This feature is that slice — see *Assumptions → Reconciliations*. No user journey above is *about* them, but each is exercised by one and carries its acceptance scenarios there: story 1 scenarios 12 to 14 for the toast, the in-flight state, the skeleton and the connection banner; story 3 scenario 9 for re-query on return; story 4 scenario 13 for a rejected write.
...
- **This feature implements the four cross-cutting UX conventions entry R2 fixed as rules.** R2's boundary defers toasts, per-screen skeletons, re-query on navigation and the connection-lost banner to "R3 or R4, whichever is built first". R3 sits on the critical path with R5 depending on it, while R4 is explicitly off that path and buildable in parallel, so R3 is built first and carries them. Under Principle I each is implemented for its own first caller here; the shared primitive behind it is extracted when a second caller lands. **If R4 is in fact built first, this reconciliation moves to R4 and FR-054 to FR-057 leave this feature** — the first thing `/speckit-clarify` should put to the team.
