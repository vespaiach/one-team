...
The rules below are fixed for the whole application by entry R2, which states them and writes no code for them because the shell has no writing or data-loading surface. Profile has both, so it implements each for its own screen and extracts no shared primitive.
...
- **The cross-cutting write and load conventions land here.** Entry R2 states toasts, per-screen skeletons, re-query on navigation and the connection-lost banner as rules and deliberately writes no code for them, leaving each to "the first slice that has one, R3 or R4 whichever is built first". Profile is a writing and data-loading surface, so it implements all four for its own screen. Under Principle I nothing is extracted as a shared primitive here — the second caller does that. If entry R3 is built first, this feature uses what R3 left rather than standing up a second implementation.
