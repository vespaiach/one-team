...
An admin opens Labels from the sidebar, presses **New label**, names it, and creates it. The label now exists once, for the whole team, and is available to be applied on any issue in any project the moment the picker is next opened. The admin can also rename an existing label — the change lands on every issue that carries it, everywhere it is shown, without touching those issues individually — and can delete a label outright, seeing first how many issues it will disappear from.
...
**Independent Test**: Sign in as an admin, open `/settings/labels` with no labels yet, create one with a name, and confirm it appears in the alphabetical list with a usage count of zero. Rename it and confirm the new name shows immediately in the list. No other story needs to exist for this to be verified.

**Acceptance Scenarios**:

1. **Given** an admin on `/settings/labels` with no labels yet, **When** they press **New label** and enter a name, **Then** one label is created, appears in the alphabetical list with a usage count of zero, and the modal closes.
2. **Given** an existing label, **When** an admin opens **Edit**, changes its name, and saves, **Then** the label's row updates in the list and the new name is what every picker and every issue carrying it shows on next render — no per-issue update is needed.
...
- A rename lands on every reference at once because a label is one row; nothing is re-tagged and no batch job runs across issues.
...
- **FR-003**: The labels page MUST list every label on the team, alphabetically by name, each row showing its name and the number of issues currently carrying it across every project.
...
- **FR-006**: **New label** MUST open a modal with a required name field; the form MUST be submittable with a name alone.
...
- **FR-009**: **Edit** MUST open the same modal, pre-populated with the label's current name, and MUST allow changing it.
- **FR-010**: Saving an edit MUST update the single label row; because a label is not duplicated per project or per issue, the new name MUST be what every issue carrying it, every board card referencing it, and every open picker shows on next render — no other row is touched.
...
- **FR-013**: Creating, renaming, or deleting a team-wide label MUST NOT write an activity record on any issue or project, and a deletion MUST NOT write an activity record on the issues it is removed from.
...
- **Label**: The team-wide category a project's issues may carry. Holds a name and nothing else. Exists exactly once across the whole installation — never scoped to a project — so the same set and the same row are what every project's issues, cards, and pickers reference. Its name is unique without regard to case.
...
- **SC-002**: Renaming a label changes how it appears everywhere it is referenced — the team's label list, every issue that carries it, every board card, and every open picker — in the single edit that made the change, with no follow-up action anywhere else.
...
- **SC-004**: Every attempt by a non-admin to reach label curation — the hidden sidebar entry, the picker's hidden "Manage labels" link, or a direct visit to `/settings/labels` — either offers no path in or resolves to Forbidden; none exposes a create, rename, or delete control.
...
- **No activity record is written for curating the set itself** (create, rename, delete) — this is the product specification's explicit choice (§3.10), not an omission, and FR-013 states it as a requirement rather than leaving it silent.
- **A label carries no colour.** The product specification's Modernist design system (§7, *Palette*) retired per-project, per-column and per-label colour: a label is told apart by name alone, the same as a project or a board column. `label.color` and any swatch picker are out of scope entirely, not deferred.
