...
| Implementation | — | **complete.** R2, R5, R6 implemented; every task in `tasks.md` is `[X]`. `FR-021`'s activity write is recorded as a deferred requirement (`tasks.md`, *`FR-021` — deferred, not tracked as an open task here*) rather than an open task, since R7's `activity` table and `writeActivity` function don't exist in code — confirmed with the user as this feature's accepted stopping point |
| 3–5 — US1, US2, Polish | [`tasks.md`](./tasks.md) T007–T051 | complete — `npm run verify` green (1409 tests, none skipped); `FR-021` deferred pending R7, per the note under Phase 4 |
