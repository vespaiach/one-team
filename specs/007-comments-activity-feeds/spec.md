...
9. **Given** a project's description edited from a 600-character value to a 900-character one, **When** the row is written, **Then** its `from_value` and `to_value` are each truncated to 200 characters rather than holding the field in full, the same rule Scenario 8 already proves for an issue's description.
...
- **FR-051**: `updateProject` MUST, within its own transaction and after determining which of its four fields differ from the stored row, write one `field_changed` activity row per differing field, each naming that field with the literal `field` value `name`, `description`, `start_date` or `target_date` — matching the column it changed — and its frozen old and new display values per FR-007 and FR-008. A call whose named values all match the stored row MUST write no row, matching the mutator's own no-op behaviour. (`OT-DATA-009`, `OT-DATA-010`)
...
- **`field_changed`'s `field` names the literal column being changed** — `title`, `description`, `column`, `priority`, `assignee`, `due_date` on an issue; `name`, `description`, `start_date`, `target_date` on a project — rather than a grouped label such as "dates" for either date field. §5's own enumeration of field names groups the two project dates together in one parenthetical, but every date field is independently editable and independently changeable in one call, so a shared label would make one row ambiguous about which date it describes. **If wrong**: renaming the value stored in `field` for the two date fields, with no schema or behavioural change elsewhere.
- **`project` carries no colour field, and this feature accordingly defines no colour-naming rule.** An earlier draft of this spec assumed a `project.colour` column, grouped with `updateProject`'s other four fields, and a palette-naming rule for it (`OT-DATA-013`) matching the convention `OT-DATA-019` states for a board column's own recolour event. [`docs/product/specifications.md`](../../docs/product/specifications.md) §7's own Palette section states the opposite plainly: "There is no per-project, per-column or per-label colour... told apart by name alone, never by a swatch" — and the shipped `project`, `board_column` and `label` tables each carry no colour column, confirming the specification's own wording over `requirements-index.md`'s `OT-DATA-013`/`OT-DATA-019`, which describe a palette nothing in the product implements. FR-051 now lists four fields, not five, and the palette-naming rule this spec previously numbered FR-009 is retired rather than renumbered, so `FR-010` onward keep the numbers every other artifact in this feature already cites. **If wrong** — if a project colour column is added later — a new FR can restore the palette-naming rule at that time, calling the same `writeActivity` primitive; nothing else in this feature changes.
