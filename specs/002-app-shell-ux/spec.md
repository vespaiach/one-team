...
- Q: If entries R3 and R4 are built concurrently, or R4 first, which entry implements FR-032 to FR-035? → A: R3, unconditionally. Ownership follows the first caller rather than the build order, as it already does for FR-013 and FR-023, and R4's spec already consumes the four as their second caller. `docs/ROADMAP.md` is amended under §5, in this branch, and this spec, its `ux-conventions` contract, its `app-shell` contract, its tasks and its conventions checklist are reconciled to it.
...
- **The implementations of toasts, skeletons, re-query on navigation and the connection-lost banner** — entry R3, which holds the first caller for each. The rules are FR-032 to FR-035.
...
- **Four conventions are stated and not implemented** — toasts, per-screen skeletons, re-query on navigation and the connection-lost banner — because the shell loads no data and its one write, sign-out, leaves the application rather than re-rendering it. The roadmap says so plainly; each lands with entry R3, which holds the first caller for it.
...
- **Downstream reach-back**: R5 fills the project-list region and fixes its ordering (`OT-UX-020`) and gives the New issue slot its first occupant; R6 gives that control its target; R11 gives Notifications its unread count; R12 gives Home its content; R3 implements FR-032 to FR-035.
