...
| `ADMIN_EMAIL` | first run only | the seeded admin's address | **absent**: seeding does not run and the app reports it, then serves normally. **Present but not a valid address**: nothing is written, the failure is named on standard error, and the process exits non-zero (`FR-073`) |
...
| `SMTP_URL` | no | the reset-mail transport: an SMTP endpoint reachable from the box that will relay for `MAIL_FROM`, and nothing more is required of it (`FR-058`) | reset requests answer identically and mail nothing; the failure is logged server-side only (`FR-033`) |
| `MAIL_FROM` | no | the address the reset mail is sent from | no mail is sent, the failure is logged, and the caller's answer is unchanged — the same behaviour as an unconfigured transport (`FR-065`) |
| `TZ` | no | the server timezone calendar dates are compared in — **no behaviour in this feature reads it**, since every value R1 stores is an instant and application logic is UTC; it exists for the calendar comparisons later entries make | defaults to the host's; §5 has the operator set it once |
...
**The mail path is part of the trust boundary.** A reset link is a bearer credential: whoever
operates the SMTP endpoint, or can read what crosses it, can complete a reset for any address it
carries. The installation requires nothing of the transport beyond reachability and a willingness to
relay, so that property is the operator's to secure, and it is stated here rather than assumed
(`FR-058`).
