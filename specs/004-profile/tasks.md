...
## ⚠️ Blocked on entry R2, and two consumptions on entry R3
...
Two obligations also reach into **entry R3**, which `docs/ROADMAP.md` names as the entry holding the
first caller for R2's four deferred conventions. The message host of `FR-033` and the connection
banner of `FR-034` are R3's to build and to mount once; this screen is their second caller and builds
neither. T018 raises into R3's host and T044 refuses a write while R3's banner is showing, so those
two tasks wait on R3 the way the rest of the screen waits on R2. Nothing else here does.
...
**Purpose**: the field table, the read and the display-name rule — each needed by two or more
stories. The message host and the connection banner are **not** here and are not this entry's:
`docs/ROADMAP.md` gives both to entry R3, which holds their first caller, and this screen consumes
what R3 mounts ([`contracts/ux-conventions.md`](./contracts/ux-conventions.md)).
...
**Checkpoint**: the record can be read and a name can be joined.
...
- [ ] T030 [Red] [US3] Write `src/features/profile/components/shown-value.test.tsx` — renders a visible label programmatically associated with its value; is not a `Button`, not focusable and not in the tab order; no press turns it into a field (`FR-024`, `FR-035`, `OT-UX-010`)
- [ ] T031 [Green] [US3] Implement `src/features/profile/components/shown-value.tsx` as a synchronous component (`FR-024`)
...
- [ ] T033 [Green] [US3] Compose `<ShownValue>` into `src/features/profile/components/profile-screen.tsx` (`FR-024`)
...
**Purpose**: this screen's half of `FR-034` — refusing a write while offline, which is its own work
even though the banner is R3's — and the end-to-end validation the runner cannot perform.

- [ ] T042 [Red] Extend `src/features/profile/profile-surface.test.ts` — no file under `src/features/profile` stands up a message host or a connection banner. This screen raises into the host entry R3 mounts and renders beneath the banner entry R3 mounts, and mounts neither itself. Observe the Red by temporarily adding a `<MessageHost />` to `src/features/profile/components/profile-screen.tsx`, then delete it (`FR-033`, `FR-034`, E-3)
- [ ] T043 [Red] Extend `src/features/profile/components/editable-field.test.tsx` — a save attempted while offline is refused **before** the action is dispatched, with the distinct wording `"Changes need a connection"`, and nothing is queued for later. The refusal takes the same rollback path as any other (`FR-034`, D-5)
- [ ] T044 [Green] Add the pre-dispatch offline check to `src/features/profile/components/editable-field.tsx` (`FR-034`)
- [ ] T045 Run every walkthrough in [`quickstart.md`](./quickstart.md) 1–12 against `npm run dev`, including the three this feature observes but does not implement — walkthrough 10's sign-out-everywhere, the cleared must-change-password flag and the return to sign-in on the next action, all R1's (`FR-030`, `SC-008`, US4 scenarios 6, 7, 9). The walkthroughs are where `SC-001`'s thirty seconds, `SC-002`'s second browser and `FR-032`'s re-query on revisit are observed — none is assertable in either Vitest project
- [ ] T046 Confirm `npm run db:generate` produces no migration and `src/db/schema.ts` and `drizzle/` are untouched (`FR-037`), and that `package.json`, `next.config.ts`, `vitest.config.mts`, `biome.json` and `tsconfig.json` carry no change — `FR-032` is satisfied by the client cache's `dynamic` stale time of `0`, which this entry gets by configuring nothing (gate 4, gate 7, D-2)
- [ ] T047 Run `npm run verify` — `style-check`, `type-check`, `test`, `build` — and confirm no test is failing or skipped (gates 5 and 8)
...
- **Polish (Phase 8)**: depends on US1's editable field existing (T044 extends it) and on `profile-surface.test.ts` existing (T042 extends what T023 created)

### User Story Dependencies

- **US1 (P1)**: Phase 2 only. Delivers the screen, the write and the route
- **US2 (P2)**: extends US1's action and `profile-surface.test.ts`, which T023 creates. Its assertions are structural and behavioural, not a new module
...
Two tasks — **T028** and **T036** — are tests belonging to a later story that constrain an earlier
...
Four tasks — **T025**, **T029**, **T039** and **T042** — prove a negative. A negative has no natural
Red, so each names the temporary violating artifact to add, observe the failure against, and delete.
Do not skip that step: a structural test that has never failed proves nothing about the tree it reads.

**Every Green in this file has a Red before it, including the one that writes no component.** T024
fills the route, which is production code, so it is preceded by the structural Red at T023 — the only
Red available for it, because neither Vitest project can render an async Server Component.

### Parallel Opportunities

- T003, T005 and T007 — three independent Red steps in three files
- T009 and T013 — the parser's tests and the field control's tests touch different files
- T034 and T037 — the auth action and the link component
- US4 can run alongside US2 and US3 once US1 is complete; the three open disjoint files
- T042 and T044 are `FR-034`'s structural and behavioural halves; neither opens a file the other does
...
2. Phase 2 — the field table, the read, the display-name rule
