...
- Q: How should the spec settle who implements the four cross-cutting UX conventions? → A: By the roadmap, which names entry R3 as the entry holding their first caller and this screen as their second. The message host and the connection banner are single app-wide instances R3 mounts and this screen consumes; the skeleton and re-query stay per-screen work this screen authors for itself under any ownership. An earlier answer here split the four by kind and gave them no owner; `docs/ROADMAP.md` was subsequently amended under §5 to settle ownership on R3, and this spec is reconciled to it (FR-031 to FR-034).
...
- **Two fields edited in quick succession** produce two independent writes; neither carries the other's value and a failure of one does not roll back the other. Their responses cannot arrive out of order: the two calls are dispatched and awaited one at a time, so the second is answered after the first.
- **The same field saved from two of the user's own sessions** resolves as last write wins. There is no version token and no conflict prompt: one person editing their own record in two tabs is the whole of the contention, and the second save either changes the value or matches it and writes nothing.
- **A re-query lands a different stored value while a field is being edited.** The edit in progress wins and is left untouched; the arriving value is what that field returns to when the edit ends. Nothing on this screen discards what the user is typing.
...
- **FR-009**: Bio MUST be optional, multi-line, and grow with its content. It MUST be plain text with no markdown: stored as typed and rendered as characters, never parsed as markup. The line breaks a stored bio carries MUST survive to the rendered output. The field MUST render at least three rows when it is empty and MUST grow to fit its content with no maximum height, the page scrolling rather than the field. (§3.12, `OT-DATA-016`)
...
- **FR-013a**: The three gestures FR-013 names are the whole set. A plain Enter MUST insert a line break in the bio, which is multi-line by FR-009, and MUST do nothing in the six single-line fields — this screen MUST NOT add a fourth save gesture, because FR-013 binds it to behave exactly as every other surface that offers in-place editing. After a save, after a rollback and after Escape, focus MUST return to the control the field replaced, so no gesture leaves focus on a removed element. (`OT-UX-009`, `SC-012`)
...
- **FR-020**: `updateOwnProfile` MUST validate every value it receives on the server — presence for the required fields, the trim, the length bound, and the avatar's scheme — whatever the browser also checked, and MUST reject rather than coerce or truncate a value that fails. A length MUST be counted in Unicode code points — the same unit the column's own constraint counts — so that no value the server accepts can violate that constraint. A submitted value that is not text at all, an absent value included, MUST be refused before it is trimmed; no such value MUST be coerced to a string. (Principle II)
...
- **FR-023**: A failed write MUST return a generic message to the caller; the underlying detail MUST stay in the server log. That message MUST read "Something went wrong. Try again.", fixed verbatim for the same reason FR-029's success wording is: leaving it free would have two surfaces write two sentences for one outcome.
...
The rules below are fixed for the whole application by entry R2, which states them and writes no code for them because the shell has no writing or data-loading surface. Profile has both, so it MUST honour each. They are not one obligation of the same kind. Two are single app-wide instances entry R3 delivers as the entry holding their first caller, which this screen consumes and MUST NOT stand up a second of; two are per-screen work this screen does for itself under any ownership.
...
- **FR-032**: A revisit to this screen through in-app navigation or a reload MUST re-query the server; nothing MUST render from a client cache. Browser back and forward restoration is the one case outside this rule: the framework preserves that tree deliberately, to hold scroll position and avoid layout shift, and this screen MUST NOT override it. This screen is the only writer of the record it shows, so a restored tree can only ever be stale against the same user's own edit in another tab, and the stored row SC-002 is about is unaffected. This is a data-fetching posture this screen adopts for itself, not a component; it holds identically whichever entry is built first. (`OT-UX-006`)
- **FR-033**: Messages raised by this screen MUST be one of the four kinds — success, info, warning, error — shown top-right, stacked, and auto-dismissing. The host that renders and stacks them MUST be a single app-wide instance living in the shell, delivered by entry R3 as the entry holding its first caller; this screen MUST raise messages into it and MUST NOT stand up a second host. Messages MUST stack newest nearest the origin, MUST auto-dismiss five seconds after they are raised, and MUST show at most three at once with any remainder queued behind them. Each raised message MUST be its own entry: repeated identical refusals MUST NOT be coalesced into one, because a second refusal is a second answer to a second attempt. (`OT-UX-016`)
- **FR-034**: A lost connection MUST raise one banner and MUST refuse writes with "Changes need a connection"; nothing MUST be queued for later. The banner MUST likewise be a single app-wide instance in the shell, one for the whole application rather than one per screen, delivered by entry R3 and consumed here on the same terms as the message host in FR-033. Refusing the write is this screen's own work; the banner is not. The banner MUST read "You're offline. Changes can't be saved.", which is its own text and distinct from the wording a blocked write is refused with. (`OT-UX-017`)
- **FR-035**: Interaction behaviour, focus management, keyboard support and ARIA semantics MUST come from the accessible component library the product standardises on, with the styling layer supplying appearance only; every field MUST carry an accessible name and its error text MUST be associated with it. The two values that are shown and not edited MUST each carry a visible label programmatically associated with the value, and MUST NOT be focusable or enter the tab order — they are text, and FR-024 keeps them from presenting as controls. (`OT-UX-018`)
...
- **Nothing is normalized to a Unicode form.** A value is stored exactly as typed once trimmed: no NFC or NFD conversion runs. Normalizing would silently alter a name its owner typed, which FR-008's "accepted as typed" rules out for the free-text fields and which nothing asks for on the two names. Counting length in code points (FR-020) matches PostgreSQL's own `char_length`, which likewise counts without normalizing, so the parser and the constraint agree on a decomposed character exactly as they agree on a composed one.
- **An avatar with no value, or one whose image does not load, renders the display name alone.** §3.12 gives the field no empty presentation. The display name is what the sidebar's user chip already falls back to for the same column, so the two agree without either being extracted, and inventing a placeholder image would be a decision no source supports (FR-012b).
- **No referrer posture is set for the third-party image an avatar names.** The browser's default applies. A deliberate posture — suppressing the referrer, or pinning it — is a product decision about what this installation discloses to an image host, and nothing in the source makes it. It is recorded here rather than settled, and it blocks nothing: the value is a link the user pasted and the screen renders it as one.
- **This screen adds no responsive rule of its own.** Its one column fills the shell's content region at every width, and horizontal scrolling at narrow widths is the shell's under entry R2. A per-screen breakpoint would be a second layout to keep in step with the skeleton FR-031 requires to match it.
...
- **The four cross-cutting UX conventions land with entry R3, and reach this screen as their second caller.** Entry R2 states toasts, per-screen skeletons, re-query on navigation and the connection-lost banner as rules and deliberately writes no code for them, leaving each to the entry holding its first caller. [`docs/ROADMAP.md`](../../docs/ROADMAP.md) names that entry as **R3** outright — R3's invitation create, its two account-state writes and its two list loads exercise all four, and this screen's one in-place field edit is their second caller — so ownership is settled by the roadmap rather than by build order, and this spec is reconciled to it. Two of the four are single app-wide instances this screen therefore **consumes and does not build**: it raises messages into the host R3 mounts and renders inside the banner R3 mounts, standing up neither. The other two stay per-screen work this screen authors for itself whatever the ownership question — a skeleton matches the layout it replaces, and R3's roster is not this layout, so there is nothing to inherit; and re-query is a data-fetching posture rather than a component. Under Principle I the second call site is where a shared primitive becomes legitimate to extract, so this is the entry at which extraction is available, not a claim on who authored it. FR-031 to FR-034 are obligations on this screen either way.
...
- Every mutation asserts the request's origin before it reads or writes anything. The convention and the helper that enforces it are entry R1's, applied here at both server entry points rather than restated as a rule of this feature's own.
- A request that resolves to no actor — no session, an expired one, or an account deactivated since the page rendered — redirects to sign-in. One helper answers for both entry points, so a mutation with no resolvable session writes nothing and returns its caller to sign-in exactly as a page request does (FR-005, `OT-SEC-015`).

### Dependencies

- **Roadmap position**: R4's declared dependencies are entries R1 and R2. Nothing depends on R4 — it sits off the critical path. Two obligations reach into R3 all the same: the message host of FR-033 and the connection banner of FR-034 are R3's to mount, so if R4 is built first those two consumptions have nothing to consume until R3 lands, exactly as the rest of the screen waits on R2. Every other requirement here holds under any build order.
...
- **Upstream from R2**: the shell, the user chip that links here, and the app-wide UX conventions this screen honours.
- **Upstream from R3**: the shell-level message host this screen raises into (FR-033) and the connection banner it renders beneath (FR-034). R3 holds the first caller for both and mounts each once; this screen is their second caller and builds neither.
- **A change of shape upstream is reconciled upstream.** This entry is specified against entries R1, R2 and R3 as those specs stand; it states no contingency for either delivering a differently shaped projection, helper or shell region. Such a change is an amendment to the entry that owns the thing, and the reconciliation belongs in that entry's spec rather than as a branch anticipated here.
