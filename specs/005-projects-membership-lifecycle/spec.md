...
**Independent Test**: Sign in as an admin against a database holding accounts but no projects, open `/projects/new`, submit a name alone, and confirm one project exists with a derived key, `active` status, five columns in their fixed order and kinds, and its own issue counter. No other story needs to exist.
...
7. **Given** a valid form, **When** the create succeeds, **Then** exactly five board columns exist for the project — Backlog, Todo, In Progress, Done, Canceled, in that order, with kinds `open`, `open`, `open`, `done`, `canceled`.
8. **Given** a valid form, **When** the create succeeds, **Then** the project's status is `active`.
...
Any signed-in person can open a project's details and read everything about it. A member of that project clicks a value — the name, the description, a date — and it becomes a field they can change on the spot. A non-member sees the same page with the same controls, disabled, each carrying the reason. The key is shown as a value and can never be edited by anyone.
...
1. **Given** any signed-in user, **When** they open a project's details, **Then** the whole record renders — key, name, description, status, dates, columns and members — with no membership check on the read.
...
The sidebar lists every project the same way for everyone — alphabetically by name, active ones first and archived ones after them, dimmed. Every project-scoped screen carries the project's name, and the pair of tabs that move between its board and its details.
...
**Independent Test**: With several projects of both statuses and mixed-case names, confirm the sidebar's order is identical for an admin, a member and a non-member, and that archived projects render after active ones and dimmed. Open project details and confirm the header carries the name and both tabs.
...
6. **Given** a signed-in user on project details, **When** the header renders, **Then** it carries the project's name, and the Board and Details tabs, with Details marked current.
...
- **FR-006**: A board column MUST belong to exactly one project and carry a name, a board position and a kind of `open`, `done` or `canceled`, fixed at creation. Column names MUST be unique within a project when folded to lower case. (`OT-INV-016`, `OT-INV-015`, §5)
- **FR-007**: Creating a project MUST seed exactly five board columns in one fixed order — Backlog, Todo, In Progress, Done, Canceled — with kinds `open`, `open`, `open`, `done`, `canceled` in that same order. (§3.3, §5)
...
- **FR-016**: `updateProject` MUST require `isMember` of the project it changes and MUST accept exactly four fields — name, description, start date and target date. It MUST NOT accept the key, which is immutable, or the status, which has its own admin-only mutator. (§2, `OT-INV-007`)
...
- **FR-044**: The Columns section MUST render as a read-only list in board order showing each column's name, kind and issue count. This feature MUST offer no control that adds, renames, reorders or deletes a column. (§3.8)
...
- **FR-054**: Every project in the sidebar MUST link to that project's board route. A name too long for the sidebar's fixed width MUST be visually truncated with the whole name still available to assistive technology, never wrapped to a second line or allowed to widen the region. (§3, The shell)
...
- **FR-056**: Every project-scoped screen's header MUST carry the project's name, and the Board and Details tab pair, with the current tab marked. A name too long for the header MUST be visually truncated on the same terms as the sidebar's, and MUST NOT displace the tab pair. (§3, The shell; §3.8)
...
- **Project** — the container work lives in. Carries a permanent uppercase key unique across the installation, a name, a markdown description, a status of `active` or `archived`, and an optional start and target date. Never carries an ordering field, a lead, or a role.
...
- **Board column** — a lane belonging to one project, carrying a name unique within that project when folded to lower case, a board position and a kind of `open`, `done` or `canceled` fixed at creation. Five are seeded with every project. Read-only in this feature.
...
- **Four contracts entry R6 needs from this feature** are stated in [`specs/006-issues-creation-detail-editing/spec.md`](../006-issues-creation-detail-editing/spec.md) under *Obligations this feature places on entries built before it*, and should be read before this feature is planned. In summary: the issue-counter row must hold the highest number yet issued for its project and be reachable from no read endpoint; the markdown implementation must stay inside this feature's own directory, because R6 is the subset's second call site and is where it is extracted; `/projects/:projectKey/details` is R6's destination after a delete; and the project header's New issue slot is filled by R6, which supplies the route it points at. R6 additionally adds one uniqueness constraint to this feature's board-column table, for a foreign key only R6 needs.
