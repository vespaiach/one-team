...
Membership **predicates** include admins; every set **derived** from membership — a UI list and a notification recipient set alike — does not. The roster on project details, the project count on Accounts, the member chips on Create project and the recipient set for a comment on a project (§3.6) read `project_member` rows only, so an admin appears in them only if they were added explicitly. Exactly two derived sets read those rows **plus** every admin: the assignee pool and the `@mention` priority group — an admin is assignable and mention-ranked in every project without holding a row, which is what makes `@mention` the way to reach an admin who holds none.
...
- Any action a user can't take **renders disabled with an inline reason.** Never a dead button, never tooltip-only. Navigation is the one exception, and it is scoped to **admin-only** screens: sidebar entries and links leading to one are hidden rather than disabled, because there is no control there to explain — only a door. A **member-only** affordance is never hidden — **New issue**, the board's inline "Add a card" composer and a card's chevron (§3.3, §3.5) render disabled for a non-member with the reason naming the project, exactly as an assigned non-member's issue page names it.
...
Reverse-chronological list. Unread rows carry a dot. Each row names the actor, the type (mentioned you / assigned you / commented), the issue or project, and a relative time; clicking it opens the issue or project, at the comment when there is one (`#comment-<id>` on that route). "Mark all read" lives in the header and is one `markAllNotificationsRead` call over the caller's own unread rows (§2), never one call per row. Three types only: `mention`, `assignment`, `comment` (issue or project). Status changes notify nobody. Activity records notify nobody — the feed is a log, not a channel. You are never notified about your own action.

**Who receives what.** A `mention` goes to each person named in the comment. An `assignment` goes to the new assignee. A `comment` on an issue goes to that issue's assignee and its creator; a `comment` on a project goes to that project's `project_member` rows — an admin holding no row is not a recipient, and `@mention`, which ranks every admin in every project (§2), is how one is reached. The actor is removed from every recipient set before the rows are written — including when they mention themselves — so `CHECK (user_id <> actor_id)` (§5) is a backstop, never the mechanism. Deactivated users are removed from every recipient set too, the same exclusion the pickers make (§3.9), so a closed account is never written a row and never mailed. One person gets one row per comment: if they would receive both, the `mention` wins.

**An edited comment notifies the people it newly names.** `updateComment` (§2) recomputes the comment's mention set and writes a `mention` row for each user in it who is neither the actor nor already holding a notification row for that comment — so correcting a mistyped mention reaches the person it was meant for, while editing the same comment repeatedly cannot mail anyone twice and cannot manufacture a second row for someone the comment already notified. Nothing else about an edit notifies anyone: rewording is not an event, and removing a mention does not withdraw a row already written.
...
**`activity`** — the append-only log behind both feeds. Exactly one of `issue_id` / `project_id`; `actor_id`; `type` (`created | field_changed | label_added | label_removed | member_added | member_removed | column_added | column_deleted | column_renamed | column_recolored | column_reordered | archived | reopened | comment`); `field`, `subject` and `from_value` / `to_value` as nullable text — for `field_changed` a single scalar (column, priority, assignee, due date, name, description, dates, colour), truncated to 200 characters so a long description never lands whole in the log; for `label_added` / `label_removed`, `to_value` / `from_value` respectively hold that one label's name, so a multi-label change is one row per label rather than one row holding a set; for `member_added` / `member_removed`, `to_value` / `from_value` respectively hold that member's display name, the actor already being `actor_id`, so the one row names both (§3.8). The five column events name their column in `subject`, which is the only thing identifying a board column in the log — `activity` holds no reference to one and gains no foreign key to a row a project may delete (§4): `column_added` and `column_deleted` carry `subject` alone, `column_renamed` carries the old and new names in `from_value` / `to_value` with `subject` holding the new one, `column_recolored` the old and new palette colours, and `column_reordered` the old and new 1-based board positions. Every one of these is a frozen display string captured at write time and bounded at 200 characters, not re-resolved on read, so a later rename of the column, user or label does not rewrite history. `comment_id` for `comment` rows; `created_at`. No `updated_at` — rows are never modified. Written by the mutators, in the same database transaction as the change they describe, so a change and its record land together or not at all.
