...
- Any action a user can't take **renders disabled with an inline reason.** Never a dead button, never tooltip-only. Navigation is the one exception: sidebar entries and links leading to an admin-only screen are hidden rather than disabled, because there is no control there to explain — only a door.
- **A member-only control is disabled, not hidden.** That exception is scoped to admin-only screens; **New issue**, the board's inline "Add a card" composer and a card's chevron are member-only controls sitting on a project-scoped screen, so a non-member sees them disabled with the reason naming the project they would need to be added to (§3.3, §3.5). The route stays guarded either way — a non-member who types the URL gets Forbidden (§3.11) — the disabled control and the server check being two layers, not two answers.
...
The project used for an `isMember` check is always derived server-side from the stored row, never from a client-supplied `project_id`. A self-scoped mutator is derived the same way: `markAllNotificationsRead` takes no id at all and writes only rows whose `user_id` is the actor's, so the one row-level read rule (§5) is also its write rule. The client runs the same predicates to disable controls; the server check is the enforcement, never the reverse.
...
greeting · three stat cards (assigned to you, due this week, unread) · **Assigned to you** (issue rows) · **Your projects** (name, status, progress — active projects only) · **Mentions** · **Recent activity** (the 20 most recent rows across every project and issue feed, newest first).
...
- **Columns belong to the project.** Every new project starts with five: Backlog · Todo · In Progress · Done · Canceled, in that order. An admin can rename them, recolour them, add more, reorder them by drag, and delete them under the restrictions in §3.8 — per project, so two projects need not agree. Each column shows its name, a count, and its colour dot.
...
A full page at `/projects/:projectKey/issues/new`, not a modal — title, description, column, priority, labels, assignee and due date is more than a modal should hold, the same reasoning as Create project (§3.7). Reached from the header's **New issue** button on any project-scoped screen (Board, issue detail, project details) and from a card's chevron on the board, which preselects that card's column. Write access follows project membership: for a non-member the **New issue** button and the board's composer render disabled with a reason naming the project (§2), and a non-member who lands on the route anyway gets Forbidden (§3.11) — the control and the route are guarded separately. Cancel returns to wherever the user came from; Create runs one `createIssue` call and, on success, navigates to the new issue's detail page.
...
Reverse-chronological list. Unread rows carry a dot. Each row names the actor, the type (mentioned you / assigned you / commented), the issue or project, and a relative time; clicking it opens the issue or project, at the comment when there is one (`#comment-<id>` on that route). "Mark all read" lives in the header. Three types only: `mention`, `assignment`, `comment` (issue or project). Status changes notify nobody. Activity records notify nobody — the feed is a log, not a channel. You are never notified about your own action.

**Who receives what.** A `mention` goes to each person named in the comment. An `assignment` goes to the new assignee. A `comment` on an issue goes to that issue's assignee and its creator; a `comment` on a project goes to that project's members — the `project_member` rows only, not every admin. A recipient set enumerates rather than authorizes, and §2's two admin-inclusive lists are the assignee pool and the `@mention` priority group alone; with no digest, no batching and no opt-out in v1 (below), mailing every admin on every project comment would be a volume no one could turn down. An admin who wants a project's comments is added to it explicitly, which is what the roster is for. The actor is removed from every recipient set before the rows are written — including when they mention themselves — so `CHECK (user_id <> actor_id)` (§5) is a backstop, never the mechanism. One person gets one row per comment: if they would receive both, the `mention` wins.

**Editing a comment** notifies the mentions the edit adds and nothing else: the recipients are the mention set after the edit minus the set before it, so a name already mentioned is not notified twice, and a name the edit removes keeps the notification it already received — a delivered notification is not withdrawn. Anyone already holding a row for that comment gets no second one; the cap is one row per comment, not one per revision. The rows are written in the same transaction as the `updateComment` (§2), with mail after commit exactly as below.
...
**Mark all read.** The header control runs one `markAllNotificationsRead` call (§2), not one call per row: a single statement stamping `read_at` over the actor's own rows where it is null, in one transaction, so the sidebar's unread count settles once and there is no partial state to report. An already-read row keeps its original `read_at` — the click marks what is unread, it does not restamp history. The write is optimistic (§4): the dots clear and the count zeroes at once, rolling back with a toast if the server refuses. The control is disabled when nothing is unread. Marking read creates no activity record and notifies nobody, for the same reason editing your own profile does not (§3.12) — activity attaches only to a project or an issue (§5, invariant 10).
...
| **Colour** | Required, one of the seven palette values (§7, *Palette*), picked from swatches. Defaults to the accent. It is the dot beside the project everywhere in the app. |
...
Everything that happens to the project record lands here: **created** · **renamed** · **description, start or target date, or colour changed** · **member added or removed** (naming both the actor and the member) · **column added, renamed, recoloured, reordered or deleted** · **archived or reopened** · **commented**.
...
Creating, renaming, recolouring, or deleting a team-wide label creates no activity record in v1, and a deletion writes none on the issues it is removed from either. Applying or removing a label on an issue by hand still creates `label_added` or `label_removed` activity on that issue.
...
**`project`** — `key` (`^[A-Z][A-Z0-9]{0,7}$` — up to 8 characters, unique, **immutable**, derived from the name at creation per §3.7), `name`, `description`, `status` (`active|archived`, default `active` — there is no planned, paused or completed state), `start_date`, `target_date`, `color`. No `sort_order`: the sidebar lists projects alphabetically by name (§3, The shell).
...
**`activity`** — the append-only log behind both feeds. Exactly one of `issue_id` / `project_id`; `actor_id`; `type` (`created | field_changed | label_added | label_removed | member_added | member_removed | column_added | column_removed | column_changed | archived | reopened | comment`); `field` and `from_value` / `to_value` as nullable text — for `field_changed` a single scalar (column, priority, assignee, due date, name, description, dates, colour), truncated to 200 characters so a long description never lands whole in the log; for `label_added` / `label_removed`, `to_value` / `from_value` respectively hold that one label's name, so a multi-label change is one row per label rather than one row holding a set. Both are frozen display strings captured at write time, not re-resolved on read, so a later rename of the column, user or label does not rewrite history. `subject`, also nullable text, names what the row is about when that is neither the issue nor the project itself: the member for `member_added` / `member_removed`, so the row names both the actor and the member as §3.8 requires, and the column for the three `column_*` types. `column_added` and `column_removed` carry `subject` alone; `column_changed` carries `subject` plus `field` (`name` | `colour` | `order`) with `from_value` / `to_value` holding that field's scalars, so a rename, a recolour and a reorder are one shape rather than three. `subject` is a frozen display string like the others and holds no foreign key — a deleted column leaves the history naming it intact, and activity keeps exactly the three cascade paths invariant 11 allows. Labels keep the `from_value` / `to_value` encoding above; a label is a value applied to an issue, where a member or a column is a subject acted upon. `comment_id` for `comment` rows; `created_at`. No `updated_at` — rows are never modified. Written by the mutators, in the same database transaction as the change they describe, so a change and its record land together or not at all.
...
Seven colours, one set, used for projects, board columns and labels alike (lowercase six-digit hex, §5). There is no per-surface palette and no free colour entry.

- accent `#5b5bd6` — the default everywhere a colour is required
- grey `#8b909a` · blue `#2f7fc4` · amber `#d4a017` · green `#3a9d5d` · red `#c8453c` · violet `#9b5de5`

The five seeded columns take grey, blue, amber, green and red, in board order (§5, `board_column`).
