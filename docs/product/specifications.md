...
**Who receives what.** A `mention` goes to each person named in the comment. An `assignment` goes to the new assignee. A `comment` on an issue goes to that issue's assignee and its creator; a `comment` on a project goes to that project's members. The actor is removed from every recipient set before the rows are written — including when they mention themselves — so `CHECK (user_id <> actor_id)` (§5) is a backstop, never the mechanism. One person gets one row per comment: if they would receive both, the `mention` wins.
