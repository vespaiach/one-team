...
- Any action a user can't take **renders disabled with an inline reason.** Never a dead button, never tooltip-only. Navigation is the one exception: sidebar entries and links leading to an admin-only screen are hidden rather than disabled, because there is no control there to explain — only a door. The exception is exactly that narrow. A member-only control on a screen a non-member can read is disabled with its reason like any other — **New issue** in the header, the board's inline "Add a card" and its chevron (§3.3, §3.5) — and the route behind it still refuses a non-member outright (§3.11). The control explains; the route enforces.
...
Reverse-chronological list. Unread rows carry a dot. Each row names the actor, the type (mentioned you / assigned you / commented), the issue or project, and a relative time; clicking it opens the issue or project, at the comment when there is one (`#comment-<id>` on that route). "Mark all read" lives in the header and is one `markAllNotificationsRead` call (§2), never a call per row: unread rows the list has not paged in still have to clear, and the client cannot enumerate what it has not fetched. Three types only: `mention`, `assignment`, `comment` (issue or project). Status changes notify nobody. Activity records notify nobody — the feed is a log, not a channel. You are never notified about your own action.

**Who receives what.** A `mention` goes to each person named in the comment. An `assignment` goes to the new assignee. A `comment` on an issue goes to that issue's assignee and its creator; a `comment` on a project goes to that project's members — the `project_member` rows alone, a recipient set being a membership **list**, and the two admin-inclusive lists §2 names are the assignee pool and the `@mention` priority group and no others. An admin who wants a project's comments adds themselves to its roster. The actor is removed from every recipient set before the rows are written — including when they mention themselves — so `CHECK (user_id <> actor_id)` (§5) is a backstop, never the mechanism. One person gets one row per comment: if they would receive both, the `mention` wins.

**Editing a comment.** `updateComment` (§2) can add mention tokens, and a mention nobody is told about is worth nothing. On an edit the mention set is recomputed, the actor and everyone already holding a row for that comment are subtracted, and whoever remains gets a `mention` row on the same terms as creation. Removing a mention retracts nothing — a row already delivered stands. Skipping anyone who already holds a row is what keeps one-row-per-comment true across an edit, and it makes the rule idempotent: no sequence of edits can produce a second row for one person on one comment.
...
**`activity`** — the append-only log behind both feeds. Exactly one of `issue_id` / `project_id`; `actor_id`; `type` (`created | field_changed | label_added | label_removed | member_added | member_removed | column_added | column_renamed | column_recolored | column_reordered | column_deleted | archived | reopened | comment`); `field` and `from_value` / `to_value` as nullable text, read according to `type`:

- **`field_changed`** — `field` names a single scalar (column, priority, assignee, due date, name, description, dates, colour) and `from_value` → `to_value` its old and new value.
- **`label_added` / `label_removed`, `member_added` / `member_removed`** — `field` is null, and `to_value` / `from_value` respectively hold that one label's name or that one member's display name. A change touching several is one row per subject, never one row holding a set.
- **The five `column_*` types** — one for each column event the project feed records (§3.8) — carry the column's name in `field`, and the three that change something carry old → new in `from_value` / `to_value`: names for a rename, palette colours for a recolour, 1-based board positions for a reorder. `column_added` and `column_deleted` leave both null, and an `updateColumn` that changes name and colour at once writes one row of each. There is deliberately no `column_id`: a reference would cascade on the column's own deletion (invariant 11) and destroy the row recording it.

Every value is a frozen display string captured at write time and truncated to 200 characters — so a long description never lands whole in the log, and a later rename of the column, user or label does not rewrite history. `comment_id` for `comment` rows; `created_at`. No `updated_at` — rows are never modified. Written by the mutators, in the same database transaction as the change they describe, so a change and its record land together or not at all.
