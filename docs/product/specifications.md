...
**`auth_attempt`** — the throttle's durable counter (§6): `kind` (`email|ip`), `subject` (the lowercased address or the IP), `failed_at`. Failures are counted over the last fifteen minutes for one subject; a successful sign-in clears that subject's rows, and rows past the window are swept by the same timer that retries mail (§7).
