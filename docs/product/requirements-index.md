...
| OT-DEC-010 | ~~**Whether a deactivated user still receives notifications.**~~ Found while reconciling this branch against main's `OT-DEC-004` resolution and **resolved** 2026-08-29 by amendment to specification §3.6 (*Who receives what*), indexed as `OT-OPS-015`: deactivated users are excluded from every recipient set, so no row is written and no mail is sent. §3.9 excluded them from the pickers only and §3.6 named no exclusion at all, so a deactivated member still holding the `project_member` rows deactivation deliberately retains (§6, `OT-SEC-013`) would have gone on being mailed at an account whose owner can no longer sign in. | §3.6 (line 209), §3.9 (line 274) | None — closed. |
