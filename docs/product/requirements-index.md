...
| OT-AUTHZ-016 | "Mark all read" MUST be one `markAllNotificationsRead` mutator requiring only self, never a call per row, and it MUST clear every unread row for the actor including those the client has not fetched. Being self-scoped, it MUST NOT be able to touch a row whose `user_id` is not the actor (`OT-AUTHZ-003`). | §2, §3.6 | Notifications |
...
| OT-DATA-019 | `member_added` / `member_removed` rows MUST carry that one member's display name — `to_value` for an addition, `from_value` for a removal — with `field` null and one row per member, on the same rule as label rows. | §3.7, §3.8, §5 | Activity, Projects |
| OT-DATA-020 | Each of the five column events the project feed records MUST have its own `activity.type` — `column_added`, `column_renamed`, `column_recolored`, `column_reordered`, `column_deleted` — carrying the column's name in `field`, and, for the three that change something, old and new in `from_value` / `to_value`. `activity` MUST NOT reference a column: the reference would cascade on that column's deletion and destroy the row recording it. | §3.8, §5 | Activity, Columns |
...
| OT-UX-021 | The hide-instead-of-disable rule MUST cover navigation to an admin-only screen and nothing else. Every other control a user cannot use — **New issue**, the board's inline "Add a card" composer and its chevron — MUST render disabled with an inline reason, and the route behind it MUST still refuse a non-member. | §2, §3.3, §3.5 | Shell, Board, Issues |
...
| OT-OPS-013 | A project `comment` notification MUST go to that project's `project_member` rows alone. Admins MUST NOT be recipients by role; the only admin-inclusive lists are the assignee pool and the `@mention` priority group (`OT-AUTHZ-007`). | §2, §3.6 | Notifications |
| OT-OPS-014 | Editing a comment MUST write a `mention` row for each newly mentioned user, excluding the actor and anyone already holding a row for that comment. Removing a mention MUST retract nothing. The rule MUST be idempotent, so no sequence of edits yields a second row for one person on one comment. | §2, §3.6 | Notifications, Comments |
...
| **Projects (record, status, members, delete)** | §3.7, §3.8, §4 | OT-SCOPE-002, OT-AUTHZ-001, OT-AUTHZ-006, OT-AUTHZ-013, OT-DATA-013, OT-DATA-015, OT-DATA-019, OT-UX-009…012, OT-OPS-010, OT-OPS-011, OT-INV-007, OT-INV-008, OT-INV-016 |
| **Board columns** | §3.3, §3.8, §4 | OT-AUTHZ-001, OT-DATA-013, OT-DATA-020, OT-UX-012, OT-OPS-010, OT-INV-005, OT-INV-006, OT-INV-012, OT-INV-014, OT-INV-015, OT-INV-016 |
...
|---|---|---|---|
| OT-DEC-001 | ~~**Column lifecycle activity has no representation in the activity schema.**~~ **Resolved** 2026-08-29 by amendment to specification §5 (`activity`), indexed as `OT-DATA-020`: the five events §3.8 already names each take their own `type`, the column's frozen name goes in `field`, and old → new goes in `from_value` / `to_value` for the three that change something. Widening `field_changed` was rejected — it is defined over the issue and project record, so a column rename would be indistinguishable from a project rename carrying the same `field`. A `column_id` reference was rejected outright: invariant 11 confines cascade deletion to a comment, issue or project, and a column reference would delete the very row recording that column's deletion. | §3.8 (line 258) vs §5 `activity` (line 377) | None — closed. |
| OT-DEC-002 | ~~**`member_added` / `member_removed` value encoding is undefined.**~~ **Resolved** 2026-08-29 by amendment to specification §5 (`activity`), indexed as `OT-DATA-019`: the analogy with labels is taken — `to_value` for an addition, `from_value` for a removal, one row per member — and the two things that analogy left silent are now stated: `field` is null, and the 200-character truncation applies to every value rather than to `field_changed` alone, since a display name joins two 200-character columns. A `member_id` reference was rejected: it contradicts the freeze rule stated in the same paragraph, and §3.12 leaves nothing to link to. | §3.8 (line 258) vs §5 `activity` (line 377) | None — closed. |
| OT-DEC-003 | ~~**"New issue" visibility for a non-member is unspecified.**~~ **Resolved** 2026-08-29 by amendment to specification §2, §3.3 and §3.5, indexed as `OT-UX-021`: disabled with an inline reason — never hidden, never enabled-then-403. Hiding was rejected because §2's exception is reasoned from there being "no control there to explain — only a door", which is false of a header button and of an inline composer, and because §3.8 already commits the neighbouring screen to looking the same to everyone. Forbidden survives as the route-level guard for a pasted or bookmarked link: the control explains, the route enforces. | §2 (line 84), §3.5 (line 187), §3.3 (line 157) | None — closed. |
| OT-DEC-004 | ~~**Whether a project-comment notification reaches every admin.**~~ **Resolved** 2026-08-29 by amendment to specification §3.6 (*Who receives what*), indexed as `OT-OPS-013`: the `project_member` rows alone. Admin-inclusive was rejected — with no digest, no batching and no opt-out in v1 (§3.6), it mails every admin on every project comment in every project, and the roster is already the opt-in an admin needs. Classifying the recipient set as a membership **list** makes §2's two named admin-inclusive lists exhaustive rather than illustrative. | §3.6 (line 209) vs §2 (line 41) | None — closed. |
| OT-DEC-005 | ~~**Whether editing a comment fires notifications for newly added mentions.**~~ **Resolved** 2026-08-29 by amendment to specification §3.6 (*Editing a comment*), indexed as `OT-OPS-014`: it fires for newly added mentions only, skipping the actor and anyone already holding a row for that comment, and retracts nothing when a mention is removed. Firing for none was rejected as a silent hole in the highest-value notification type. Skipping whoever already holds a row is what keeps §3.6's one-row-per-comment rule true across an edit, and it makes the rule idempotent under repeated edits. | §3.6 (lines 209–211), §2 (line 92) | None — closed. |
...
| OT-DEC-007 | ~~**"Mark all read" has no mutator.**~~ **Resolved** 2026-08-29 by amendment to specification §3.6 and §2 (*Requires only self*), indexed as `OT-AUTHZ-016`: one `markAllNotificationsRead` call. N calls was rejected as incorrect rather than merely wasteful — unread rows the list has not paged in still have to clear, and the client cannot enumerate what it has not fetched. The mutator is self-scoped, so it can only ever touch rows whose `user_id` is the actor (`OT-AUTHZ-003`). | §3.6 (line 207) vs §2 (line 93) | None — closed. |
