...
| **Board columns** | §3.3, §3.8, §4 | OT-AUTHZ-001, OT-DATA-013, OT-UX-012, OT-OPS-010, OT-INV-005, OT-INV-006, OT-INV-012, OT-INV-014, OT-INV-015, OT-INV-016 |
| **Board (grouping, drag, ordering)** | §3.3, §4 | OT-SCOPE-004, OT-AUTHZ-007, OT-AUTHZ-015, OT-DATA-017, OT-UX-008, OT-OPS-008, OT-OPS-009, OT-OPS-011, OT-INV-004 |
| **Issues (detail, create, edit)** | §3.4, §3.5, §4 | OT-SCOPE-003, OT-AUTHZ-007, OT-AUTHZ-015, OT-DATA-004, OT-DATA-012, OT-DATA-015, OT-DATA-017, OT-UX-008…011, OT-INV-001…004, OT-INV-009 |
...
|---|---|---|---|
| OT-DEC-001 | **Column lifecycle activity has no representation in the activity schema.** §3.8 requires the project feed to record "column added, renamed, recoloured, reordered or deleted", but §5's `activity.type` enum is `created \| field_changed \| label_added \| label_removed \| member_added \| member_removed \| archived \| reopened \| comment` — no column type — and `field_changed` is defined as a single scalar over issue and project record fields (column, priority, assignee, due date, name, description, dates, colour). "Column added", "deleted" and "reordered" have no from → to scalar, and `activity` carries no column reference. | §3.8 (line 254) vs §5 `activity` (line 373) | Blocks the activity writer and the project feed renderer. Either the enum widens or the encoding for five distinct events must be specified. |
| OT-DEC-002 | **`member_added` / `member_removed` value encoding is undefined.** §3.8 requires these rows to name both the actor and the member, but §5 specifies `from_value` / `to_value` only for `field_changed` and for `label_added` / `label_removed`. | §3.8 (line 254) vs §5 `activity` (line 373) | Blocks the member mutators and feed rendering; inferable by analogy with labels, but not stated. |
| OT-DEC-003 | **"New issue" visibility for a non-member is unspecified.** §2 makes hide-instead-of-disable an exception for navigation to **admin-only** screens; Create issue is member-only, not admin-only. §3.5 says a non-member landing on the route gets Forbidden, implying the control is reachable — which conflicts with the general rule that an unavailable action renders disabled with a reason. Same question for the board's inline "Add a card" composer and the card chevron. | §2 (line 84), §3.5 (line 185), §3.3 (line 157) | Three viable renderings (hidden / disabled-with-reason / enabled-then-403). Affects header, board and Forbidden behaviour. |
| OT-DEC-004 | **Whether a project-comment notification reaches every admin.** §3.6 says a project `comment` goes to "that project's members". §2 states membership *lists* exclude admins and names exactly two admin-inclusive lists — the assignee pool and the `@mention` priority group — neither of which is this one. The recipient set is not classified either way. | §3.6 (line 205) vs §2 (line 41) | Determines whether every admin is mailed on every project comment. Materially changes notification volume. |
| OT-DEC-005 | **Whether editing a comment fires notifications for newly added mentions.** §3.6 defines recipients per comment and delivery "in the same transaction as the change that caused it", but describes only creation. `updateComment` exists in §2 and can add mention tokens. | §3.6 (lines 205–207), §2 (line 92) | Blocks `updateComment`: fire for new mentions, fire for none, or re-diff the mention set. |
| OT-DEC-006 | **Whether reset requests share the sign-in throttle counter.** §6 says reset requests are "throttled the same way" and both are counted in `auth_attempt`, where "a successful sign-in clears that subject's rows". If the counter is shared, requesting resets can lock a user out of sign-in, and signing in clears the reset throttle. | §6 (line 424), §3.1 (line 137) | Blocks `auth_attempt` schema use — `kind` is `email \| ip` with no discriminator for which flow recorded the row. |
| OT-DEC-007 | **"Mark all read" has no mutator.** §3.6 puts "Mark all read" in the header; §2's per-mutator inventory lists only `markNotificationRead` under "requires only self". | §3.6 (line 203) vs §2 (line 93) | Unspecified whether this is N calls or an unlisted `markAllNotificationsRead`. Affects the mutator surface and its authorization row. |
...
| OT-DEC-009 | **Whether the first-run seeded `ADMIN_PASSWORD` is subject to the password policy.** §3.1 sets a global policy (≥12 characters, blocklist); §6 describes seeding from the environment without saying whether the value is validated or what happens when it fails. | §6 (line 414) vs §3.1 (line 140) | Blocks bootstrap: validate and refuse to start, validate and warn, or accept unchecked. |
