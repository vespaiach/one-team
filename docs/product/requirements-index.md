...
| OT-DATA-020 | `member_added` / `member_removed` rows MUST carry that one member's display name — `to_value` for an addition, `from_value` for a removal — with `field` null and one row per member, on the same rule as label rows. The 200-character truncation MUST apply to every value, a display name joining two 200-character columns. | §3.7, §3.8, §5 | Activity, Projects |
...
| **Board (grouping, drag, ordering)** | §3.3, §4 | OT-SCOPE-004, OT-AUTHZ-007, OT-AUTHZ-015, OT-DATA-017, OT-DATA-018, OT-UX-008, OT-OPS-008, OT-OPS-009, OT-OPS-011, OT-INV-004 |
| **Issues (detail, create, edit)** | §3.4, §3.5, §4 | OT-SCOPE-003, OT-AUTHZ-007, OT-AUTHZ-015, OT-DATA-004, OT-DATA-012, OT-DATA-015, OT-DATA-017, OT-DATA-018, OT-UX-008…011, OT-INV-001…004, OT-INV-009 |
