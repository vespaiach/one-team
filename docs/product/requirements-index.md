...
| OT-DATA-019 | Column lifecycle MUST be recorded by five `activity` types — `column_added`, `column_renamed`, `column_recoloured`, `column_reordered`, `column_deleted` — each with `field` null and the column named by a frozen display string in `from_value` / `to_value` per type. `activity` MUST NOT carry a foreign key to the column, so deleting one cannot cascade away the record of its own deletion. | §5, §3.8 | Activity, Columns |
| OT-DATA-020 | `member_added` / `member_removed` MUST hold that member's display name as a frozen string in `to_value` / `from_value` respectively, the actor coming from `actor_id`. | §5, §3.8 | Activity, Projects |
...
| OT-UX-021 | A member-only control MUST render disabled with an inline reason for a non-member, never hidden — the header's New issue button, the board's inline composer and its chevron included. Hiding MUST stay confined to navigation to an admin-only screen. The route guard MUST hold regardless, so direct navigation still reaches Forbidden. | §2, §3.3, §3.5 | Board, Issues, Shell |
...
|---|---|---|---|
| OT-DEC-001 | ~~**Column lifecycle activity has no representation in the activity schema.**~~ **Resolved** 2026-08-29 by amendment to specification §5 (`activity`), indexed as `OT-DATA-019`: the enum widens by five types — `column_added`, `column_renamed`, `column_recoloured`, `column_reordered`, `column_deleted` — each assigning `from_value` / `to_value` per type, the way `label_added` / `label_removed` already do, with `field` null throughout. Encoding the five through `field_changed` was rejected: it is defined as a scalar over issue and project record fields, and three of the five have no from → to scalar to give it. A `column_id` reference was rejected outright — it would cascade on delete and erase the record of its own deletion, leaving the one column event that most needs a log unable to keep one, which is also why the existing types freeze names rather than reference rows. `column_reordered` freezes only the moved column's name, a drag having no from → to worth keeping. | §3.8 (line 258) vs §5 `activity` (line 377) | None — closed. |
| OT-DEC-002 | ~~**`member_added` / `member_removed` value encoding is undefined.**~~ **Resolved** 2026-08-29 by amendment to specification §5 (`activity`), indexed as `OT-DATA-020`: `to_value` / `from_value` hold that member's display name as a frozen string, exactly as the label types hold a label's, the actor coming from `actor_id`. The analogy is made explicit rather than left inferable; §5 already assumed it in requiring that a later rename of a *user* not rewrite history. | §3.8 (line 258) vs §5 `activity` (line 377) | None — closed. |
| OT-DEC-003 | ~~**"New issue" visibility for a non-member is unspecified.**~~ **Resolved** 2026-08-29 by amendment to specification §3.5 (write access) and §3.3 (*Add a card*), indexed as `OT-UX-021`: the header button, the board's inline composer and its chevron all render disabled with an inline reason, and the route keeps its Forbidden guard. Hiding was rejected because §2 confines it to admin-only navigation and reasons it from there being no control to explain — here there is one, and the disabled reason is the same sentence the rail and the rejected-write toast already carry. The two treatments turned out not to conflict: the disabled control is the client affordance, the route guard is the enforcement, which is the two-layer model §2 states. | §2 (line 84), §3.5 (line 187), §3.3 (line 157) | None — closed. |
