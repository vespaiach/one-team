...
| OT-AUTHZ-016 | Marking notifications read MUST be self-scoped. `markNotificationRead` MUST verify the named row's `user_id` against the actor; `markAllNotificationsRead` MUST accept no id at all and write only rows whose `user_id` is the actor's, so the one row-level read rule (`OT-AUTHZ-003`) is also its write rule. "Mark all read" MUST be one call over the actor's unread rows, never one call per row. | §2, §3.6 | Notifications |
...
| OT-DATA-019 | `activity` MUST carry a nullable `subject` naming what a row is about where that is neither the issue nor the project — a frozen display string on the same terms as `from_value` / `to_value`, and never a foreign key, so a deleted column leaves the history naming it intact and `activity` keeps exactly the three cascade paths `OT-INV-011` allows. | §3.8, §5 | Activity, Columns |
| OT-DATA-020 | `activity.type` MUST include `column_added`, `column_removed` and `column_changed`. The first two MUST carry `subject` alone; `column_changed` MUST carry `subject` plus `field` (`name \| colour \| order`) with `from_value` / `to_value` holding that field's scalars. `member_added` / `member_removed` MUST name the member in `subject` and the actor in `actor_id`, so one row names both. Labels MUST keep their `from_value` / `to_value` encoding. | §3.8, §5 | Activity, Columns, Projects |
...
| OT-UX-021 | A member-only control — **New issue**, the board's inline "Add a card" composer, a card's chevron — MUST render disabled with an inline reason naming the project, and MUST NOT be hidden. `OT-UX-003`'s hide-instead-of-disable rule MUST stay scoped to navigation to admin-only screens. The route MUST stay guarded independently of the control. | §2, §3.3, §3.5 | Board, Issues, Shell |
...
| OT-OPS-013 | A `comment` notification on a project MUST go to that project's `project_member` rows only and MUST NOT reach every admin. An admin MUST be added to the project explicitly to receive them. | §2, §3.6 | Notifications |
| OT-OPS-014 | Editing a comment MUST notify only the mentions the edit adds — the mention set after the edit minus the set before it. A name already mentioned MUST NOT be notified twice; a mention the edit removes MUST NOT withdraw a delivered notification; anyone already holding a row for that comment MUST NOT receive a second one. The rows MUST be written in the same transaction as `updateComment`. | §2, §3.6 | Notifications, Comments |
...
| **Board columns** | §3.3, §3.8, §4 | OT-AUTHZ-001, OT-DATA-013, OT-DATA-019, OT-DATA-020, OT-UX-012, OT-OPS-010, OT-INV-005, OT-INV-006, OT-INV-012, OT-INV-014, OT-INV-015, OT-INV-016 |
...
Contradictions, ambiguities and missing decisions found in the source. **None are resolved here** — the decision belongs to the specification's owner, and a row is struck and marked resolved only once the source itself has been amended. Each open row names the passages that disagree. No row is open: every entry below has been amended into the source and struck.

| ID | Conflict or gap | Source | Impact if unresolved |
|---|---|---|---|
| OT-DEC-001 | ~~**Column lifecycle activity has no representation in the activity schema.**~~ **Resolved** 2026-08-29 by amendment to specification §5 (`activity`), indexed as `OT-DATA-019` and `OT-DATA-020`: the enum gains `column_added`, `column_removed` and `column_changed`, and a nullable frozen-text `subject` names the column, so a rename, a recolour and a reorder share one shape rather than needing three more types. A `column_id` reference was rejected — it gives `activity` a fourth cascade path and destroys a column's history along with the column, against `OT-INV-011`. Widening `field_changed` was rejected because an add, a delete and a reorder have no from → to scalar to carry. | §3.8 (line 258) vs §5 `activity` (line 377) | None — closed. |
| OT-DEC-002 | ~~**`member_added` / `member_removed` value encoding is undefined.**~~ **Resolved** 2026-08-29 by the same amendment, indexed as `OT-DATA-020`: the member's frozen display name goes in `subject` and the actor stays `actor_id`, so one row names both as §3.8 requires. The label analogy — the name in `to_value` / `from_value` — was rejected for coherence with the column types resolved in `OT-DEC-001`: a label is a value applied to an issue, where a member is a subject acted upon. | §3.8 (line 258) vs §5 `activity` (line 377) | None — closed. |
| OT-DEC-003 | ~~**"New issue" visibility for a non-member is unspecified.**~~ **Resolved** 2026-08-29 by amendment to specification §2 (*Consequences the UI must handle*), §3.3 and §3.5, indexed as `OT-UX-021`: the control renders disabled with an inline reason naming the project, and the board's inline composer and card chevron follow it. Hiding was rejected because §2 scopes that exception to navigation into admin-only screens, "because there is no control there to explain" — a header button on a member-only screen is a control, not a door. §3.5's Forbidden is not in conflict and stays: the disabled control and the route guard are two layers, not two answers. | §2 (line 84), §3.5 (line 187), §3.3 (line 157) | None — closed. |
| OT-DEC-004 | ~~**Whether a project-comment notification reaches every admin.**~~ **Resolved** 2026-08-29 by amendment to specification §3.6 (*Who receives what*), indexed as `OT-OPS-013`: `project_member` rows only. A recipient set enumerates rather than authorizes, and §2 names exactly two admin-inclusive lists, neither of them this one. Including every admin was rejected on volume — §3.6 ships with no digest, no batching and no opt-out, so an admin mailed on every project comment everywhere would have no way to turn it down. An admin who wants them is added to the project, which is what the roster is for. | §3.6 (line 209) vs §2 (line 41) | None — closed. |
| OT-DEC-005 | ~~**Whether editing a comment fires notifications for newly added mentions.**~~ **Resolved** 2026-08-29 by amendment to specification §3.6, indexed as `OT-OPS-014`: the edit notifies the mention set after it minus the set before it, and nothing else. Firing for none was rejected — adding a forgotten `@mention` is the ordinary reason to edit, and silence there fails the product's only notification channel. Re-notifying the whole set was rejected against `OT-OPS-006`, whose one row per comment the amendment reads as one per comment rather than one per revision; a mention the edit removes keeps what it already received, since a delivered notification is not withdrawn. | §3.6 (lines 209–211), §2 (line 92) | None — closed. |
...
| OT-DEC-007 | ~~**"Mark all read" has no mutator.**~~ **Resolved** 2026-08-29 by amendment to specification §2 (*Write rules per mutator*) and §3.6, indexed as `OT-AUTHZ-016`: `markAllNotificationsRead` joins *requires only self*, taking no id and writing one statement over the actor's own unread rows. N calls to `markNotificationRead` were rejected — the client would own the definition of "all" and could mark only what it had fetched, a partial failure would leave §4's rejected-write toast nothing honest to name, and the sidebar's unread count would settle only after the last call landed. Already-read rows keep their original `read_at`. | §3.6 (line 207) vs §2 (line 93) | None — closed. |
