...
| OT-AUTHZ-016 | `markAllNotificationsRead` MUST exist as a mutator requiring only self, clearing the caller's own unread notification rows in one statement. "Mark all read" MUST NOT be N `markNotificationRead` calls. | §3.6, §2 | Notifications |
...
| OT-DATA-019 | `activity.type` MUST carry `column_added`, `column_deleted`, `column_renamed`, `column_recolored` and `column_reordered`, and `activity` MUST carry a nullable `subject` naming the board column a column event is about. `activity` MUST NOT hold a foreign key to `board_column`. | §5, §3.8 | Activity, Columns |
| OT-DATA-020 | `member_added` / `member_removed` MUST carry the member's display name, frozen at write time, in `to_value` / `from_value` respectively, the actor coming from `actor_id`, so the one row names both. | §5, §3.8 | Activity, Projects |
...
| OT-UX-021 | A member-only affordance MUST render disabled with an inline reason naming the project, never hidden — the header's **New issue**, the board's "Add a card" composer and a card's chevron alike. Hiding MUST stay scoped to navigation leading to an admin-only screen, and the route MUST still answer Forbidden to a non-member who reaches it directly. | §2, §3.3, §3.5 | Shell, Board, Issues |
...
| OT-OPS-013 | A `comment` notification on a project MUST go to that project's `project_member` rows only; an admin holding no row MUST NOT be a recipient. | §3.6, §2 | Notifications |
| OT-OPS-014 | `updateComment` MUST write a `mention` notification for each user the edited body names who is neither the actor nor already holding a notification row for that comment, and MUST notify nobody else. | §3.6, §2 | Notifications, Comments |
| OT-OPS-015 | Deactivated users MUST be excluded from every notification recipient set, so a closed account is never written a row and never mailed. | §3.6, §3.9 | Notifications |
...
| **Projects (record, status, members, delete)** | §3.7, §3.8, §4 | OT-SCOPE-002, OT-AUTHZ-001, OT-AUTHZ-006, OT-AUTHZ-013, OT-DATA-013, OT-DATA-015, OT-UX-009…012, OT-DATA-020, OT-OPS-010, OT-OPS-011, OT-INV-007, OT-INV-008, OT-INV-016 |
| **Board columns** | §3.3, §3.8, §4 | OT-AUTHZ-001, OT-DATA-013, OT-UX-012, OT-OPS-010, OT-INV-005, OT-INV-006, OT-INV-012, OT-INV-014, OT-INV-015, OT-INV-016, OT-DATA-019 |
...
| **Comments and activity** | §3.4, §3.8, §2, §5 | OT-AUTHZ-008, OT-AUTHZ-009, OT-AUTHZ-014, OT-DATA-009…011, OT-DATA-014, OT-DATA-016, OT-DATA-019, OT-DATA-020, OT-UX-013…015, OT-OPS-014, OT-INV-010, OT-INV-011 |
...
|---|---|---|---|
| OT-DEC-001 | ~~**Column lifecycle activity has no representation in the activity schema.**~~ **Resolved** 2026-08-29 by amendment to specification §5 (`activity`), indexed as `OT-DATA-019`: the enum widens by five — `column_added`, `column_deleted`, `column_renamed`, `column_recolored`, `column_reordered` — and a nullable `subject` names the column each row is about, with `from_value` / `to_value` carrying old and new names, palette colours, or 1-based board positions. A foreign key to `board_column` was rejected: §4 has a column delete cascade to nothing, and a reference would either block the delete or erase the history of it, where `subject` freezes the name under the rule §5 already applies to column, user and label names. Overloading `field` to carry the column's identity was rejected as a stringly-typed alternative to one nullable column. | §3.8 (line 258) vs §5 `activity` (line 377) | None — closed. |
| OT-DEC-002 | ~~**`member_added` / `member_removed` value encoding is undefined.**~~ **Resolved** 2026-08-29 by amendment to specification §5 (`activity`), indexed as `OT-DATA-020`: `to_value` / `from_value` hold the member's display name frozen at write time, the actor coming from `actor_id`, so the one row names both as §3.8 requires — the encoding labels already use. Re-resolving the name from a `user_id` on read was rejected: §5 freezes display strings so history is not rewritten by a later rename, and it names the user rename as one of the three cases it means. | §3.8 (line 258) vs §5 `activity` (line 377) | None — closed. |
| OT-DEC-003 | ~~**"New issue" visibility for a non-member is unspecified.**~~ **Resolved** 2026-08-29 by amendment to specification §2, §3.3 and §3.5, indexed as `OT-UX-021`: disabled with an inline reason naming the project, never hidden — the header button, the board's "Add a card" composer and the card chevron alike — with the route still answering Forbidden to anyone who reaches it directly. Hiding was rejected because §2 scopes that exception to **admin-only** navigation, and because membership is the boundary this product insists elsewhere on explaining: an assigned non-member's issue page already names the project they would need to join. Leaving the control enabled to fail at the route was rejected as the dead button §2 forbids. | §2 (line 84), §3.5 (line 187), §3.3 (line 157) | None — closed. |
| OT-DEC-004 | ~~**Whether a project-comment notification reaches every admin.**~~ **Resolved** 2026-08-29 by amendment to specification §3.6 (*Who receives what*) and §2, indexed as `OT-OPS-013`: `project_member` rows only, an admin holding no row is not a recipient. Including every admin was rejected because §2's admin-inclusive exception is closed at two sets and rests on assignability and mention-ranking, neither of which is delivery; because v1 offers no opt-out (§3.6) and the set would grow with every project created; and because `@mention` already ranks every admin first in every project, which is the designed way to reach one. The amendment generalises §2 from membership *lists* to every set derived from membership, so the next derived set does not reopen the question. | §3.6 (line 209) vs §2 (line 41) | None — closed. |
| OT-DEC-005 | ~~**Whether editing a comment fires notifications for newly added mentions.**~~ **Resolved** 2026-08-29 by amendment to specification §3.6, indexed as `OT-OPS-014`: `updateComment` writes a `mention` row for each user the new body names who is neither the actor nor already holding a row for that comment. Firing for nothing was rejected because a mistyped mention would then be unfixable — the person it meant to name would never learn of it. Re-firing the whole set on every edit was rejected because it would mail the same people repeatedly and could hand one person two rows for one comment, which §3.6's one-row-per-comment rule forbids. | §3.6 (lines 209–211), §2 (line 92) | None — closed. |
...
| OT-DEC-007 | ~~**"Mark all read" has no mutator.**~~ **Resolved** 2026-08-29 by amendment to specification §3.6 and §2, indexed as `OT-AUTHZ-016`: one `markAllNotificationsRead` mutator, requiring only self, clearing the caller's own unread rows in one statement. N `markNotificationRead` calls were rejected — the count is unbounded, the result is not atomic, and a partial failure would leave the header's unread count disagreeing with the list it heads. | §3.6 (line 207) vs §2 (line 93) | None — closed. |
...
| OT-DEC-010 | ~~**Whether a deactivated user still receives notifications.**~~ Found while resolving `OT-DEC-004` and **resolved** 2026-08-29 in the same amendment to specification §3.6, indexed as `OT-OPS-015`: deactivated users are excluded from every recipient set, so no row is written and no mail is sent. §3.9 excluded them from the pickers only and §3.6 named no exclusion at all, so a deactivated member still holding the `project_member` row that `OT-SEC-013` deliberately retains would have been mailed at a closed account on every comment. | §3.6 (line 209), §3.9 (line 272) | None — closed. |
