...
| **Constitution** | [`.specify/memory/constitution.md`](../.specify/memory/constitution.md) — **v1.0.0, ratified 2026-08-27**. Seven principles plus Technology Constraints, Development Workflow and Governance. It binds every child spec (§1.1); it does not govern this roadmap's decomposition. |
...
Constitution v1.0.0 changes nothing about the decomposition below — no slice is added, removed or reordered by it. It binds each child spec's plan and implementation through the seven gates in its Development Workflow section. Three of those shape how a slice is *specified*, not merely how its diff is reviewed:

- **Test-First (VII, non-negotiable)** — a failing test, observed failing for the intended reason, precedes every line of production code. A child spec's acceptance scenarios are therefore its Red step, written before implementation rather than recorded after it. Blocked by `RD-004`.
- **Built-in over third-party (IV)** — every library the specification's §7 stack names needs team approval recorded *before* it is installed. Blocked by `RD-005`.
...
| **R12** | Home roll-up | One read-only landing page answering "what is mine, and what just happened". | **In:** `/home`'s content — the greeting, the three stat cards, Assigned to you, Your projects with the progress figure and its zero-denominator rule, Mentions, and Recent activity as the 20 most recent rows across every project and issue feed; the headerless layout that makes Home the shell's one exception. **Deferred:** no writes and no new mutators — Home reads only. The Mentions section's source is `RD-003`. | `OT-AUTHZ-002`, `OT-DATA-004`, `OT-UX-001`, `-005`…`-007`, `OT-INV-014` | R7, R10, R11 | `planned` | — |

### Cross-cutting requirements

These are established by the slice named and then hold for **every** later slice, rather than being repeated in each row: `OT-DATA-001`…`-003` and `OT-DATA-005`, `-006` (R1); `OT-AUTHZ-004`, `-005`, `-012` (R1–R2); `OT-UX-002`, `-005`…`-008`, `-016`…`-019` (R2); `OT-SCOPE-001`, `-005` (the whole epic — no slice may build an out-of-scope feature).
...
Not resolved here. Each belongs to the specification's owner, and each blocks the slice named.
...
| `OT-DEC-006` | Whether reset requests share the sign-in throttle counter. | R1 |
| `OT-DEC-007` | "Mark all read" has no mutator in the §2 inventory. | R11 |
| `OT-DEC-008` | The deactivated sign-in message "names an admin" — which admin, and may it be disclosed unauthenticated. | R1 |
| `OT-DEC-009` | Whether the seeded `ADMIN_PASSWORD` is subject to the password policy. | R1 |
...
| `RD-002` | **Where a newly created issue lands in the order is unspecified.** §3.3 makes drop position the only ordering input and gives `moveIssue` the write, and §3.3 says an issue has one `sort_order` across the whole project. But `createIssue` must produce an initial index, and neither §3.3's "Add a card" nor §3.5's Create issue page says where the new issue sits — head of its column, foot of its column, or head or foot of the project-wide sequence. "Add a card" sitting at the foot of a column is a placement of the control, not a stated ordering rule. | §3.3 (lines 157, 163–165), §3.5 (line 197) | Blocks `createIssue` in R6 and the board's first render in R10. Four viable rules, each giving a different board on the next load. | `open` |
| `RD-003` | **Home's "Mentions" section has no defined source.** §3.2 lists it among Home's sections without saying what it reads — unread `mention` notifications, all `mention` notifications, or comments carrying the viewer's token — nor how many rows, nor whether read ones persist. Every other Home section is given its query or its formula. | §3.2 (line 146) vs §3.6 (lines 203–205) | Blocks R12's Mentions query. Notifications are readable only by their own `user_id` (`OT-AUTHZ-003`), so the choice also decides whether Home reads the notification table at all. | `open` |
| `RD-004` | **No test runner is configured, and Principle VII is non-negotiable.** `package.json` on `main` carries no `test` script and no test framework. The constitution flags this against itself as `TODO(TEST_RUNNER)` and states that Principle VII cannot be enforced in CI until the team selects one. Principle VII forbids writing any production code without a test first observed failing for the intended reason, and Development Workflow gate 1 requires a reviewer to see that evidence. Under Principle IV, Node's built-in `node:test` is the preferred candidate; anything else needs recorded approval. | Constitution — Principle VII, Technology Constraints, Development Workflow gate 1 — vs `package.json` | Blocks the implement phase of **every** slice, R1 first: there is no way to produce a Red step, so no production code may be written at all. Choose a runner and add a `test` script before R1 is planned. | `open` |
| `RD-005` | **The specification's stack is not the constitution's approved stack.** Technology Constraints names Next.js 16, React 19, TypeScript, Tailwind v4 and Biome, and Principle IV requires team approval recorded *before* any further dependency is added. Specification §7 mandates Drizzle ORM, PostgreSQL, `@node-rs/argon2`, `nodemailer`, `uuidv7`, `fractional-indexing` and React Aria Components — none of them named in that section. `react-aria-components`, `drizzle-orm`, `postgres`, `drizzle-kit`, `@next/env` and `server-only` are already installed on `main` without the section being amended. | Constitution — Principle IV, Technology Constraints — vs specification §7 | Blocks R1 at the point it installs `@node-rs/argon2`, `nodemailer` and `uuidv7`, and R10 at `fractional-indexing`, and leaves the already-installed set unrecorded. Either amend Technology Constraints to adopt §7's stack wholesale, or record approval per dependency. | `open` |
