...
| **Constitution** | [`.specify/memory/constitution.md`](../.specify/memory/constitution.md) — **v1.1.0**, ratified 2026-08-27, last amended 2026-08-27. Seven principles plus Technology Constraints, Development Workflow and Governance. It binds every child spec (§1.1); it does not govern this roadmap's decomposition. |
...
- **Built-in over third-party (IV)** — every dependency needs team approval recorded *before* it is installed. Constitution v1.1.0 carries that record as a table in Technology Constraints, and it is the complete set: a library absent from it needs its own amendment (`RD-005`, resolved).
