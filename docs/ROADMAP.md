...
- **Built-in over third-party (IV)** — every dependency needs team approval recorded *before* it is installed. Constitution v1.1.0 carries that record as a table in Technology Constraints, and it is the complete set: a library absent from it needs its own amendment (`RD-005`, resolved; one such library is open as `RD-006`).
...
| `RD-006` | **An installed dependency is absent from the approved set.** `babel-plugin-react-compiler` is in `package.json` and enabled by `reactCompiler: true` in `next.config.ts`, but appears nowhere in Technology Constraints — neither in its prose stack line nor in the approved-dependency table that `RD-005` added. Principle IV requires approval recorded *before* installation, and the table states it is the complete set, so the two disagree about a dependency already running. | Constitution — Principle IV, Technology Constraints — vs `package.json`, `next.config.ts` | Blocks no slice: the React Compiler is build tooling, not a slice's dependency. It leaves the approved set inaccurate, so the next child spec that checks gate 4 against it checks against a list the tree already contradicts. Record it by amendment, or remove the plugin. | `open` |
