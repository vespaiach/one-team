...
| **Constitution** | [`.specify/memory/constitution.md`](../.specify/memory/constitution.md) — **v1.4.1**, ratified 2026-08-27, last amended 2026-08-28. Governance, the amendment procedure, the version history and the project's decision records, plus pointers to the principles, technology constraints and change gates `AGENTS.md` now hosts. It binds every child spec (§1.1); it does not govern this roadmap's decomposition. |
...
| 2026-08-28 | Authority reorganised by constitution amendment **v1.3.0** — the seven core principles move to [`AGENTS.md`](../AGENTS.md) verbatim, and `AGENTS.md` becomes the primary source for how code is built. Numerals and names are unchanged, so every `Principle` citation in this document still resolves. The constitution retains Technology Constraints, Development Workflow and Governance, and remains the principles' version record. §1's authority rows reconciled. Amendment **v1.3.1** then condensed each principle to a single paragraph, and **v1.4.0** moved Technology Constraints and Development Workflow after them — the approved-dependency table and the eight change gates now live in `AGENTS.md`, with gate 5 corrected from the nonexistent `npm run lint` to `npm run style-check`. Obligations, gate numbering and citations unchanged throughout. Amendment **v1.4.1** then condensed that hosted guidance — restatements of II, IV and VII removed, general-practice material dropped, the `--passWithNoTests` caveat on gate 8 and Biome's mechanical enforcement of VI recorded. Decomposition unchanged; no slice added, removed or reordered. |
