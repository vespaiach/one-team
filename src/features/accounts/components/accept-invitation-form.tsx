...
function NameField({ name, label, isInvalid }: { name: string; label: string; isInvalid: boolean }) {
  return (
    <TextField
      ...
      <Input className="h-[36px] w-full border border-[var(--color-divider)] bg-[var(--color-surface)] px-[10px] py-1 text-[14px] text-[var(--color-text)] caret-[var(--color-accent)] data-[invalid]:border-[var(--color-accent)]" />
...
export function AcceptInvitationForm({
  ...
  return (
    <Form
      ...
      <h1 className="text-h1 mb-1">You've been invited</h1>
